 ...
 * @Last Modified by: zp
 * @Last Modified time: 2020-06-10 11:04:52
 */
...
export function checkAuth(userId, authCode, isTop) {
  // const user = getCurrentUser();
  // const { admin, userId: currUserId, authorys, } = user;
  // const temp = admin || authorys.includes(authCode);
  // if (isTop) {
  //   return temp;
  // }

  // return (
  //   userId === currUserId || temp
  // );
  return true;
}

export function isCurrUser(userId) {
  const user = getCurrentUser();
  const { userId: currUserId } = user;
  return userId === currUserId ? 'true' : 'false';
}
